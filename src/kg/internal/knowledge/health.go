...
func CollectHealthMetrics(store *Store, projectID string) (*HealthMetrics, error) {
	...
	for result.HasNext() {
		...
		countVal, _ := row.GetValue(1)
		entityType, _ := typeVal.(string)
		if entityType == "" {
			entityType = "(untyped)"
		}
		m.EntitiesByType[entityType] = intFromCount(countVal)
	}
	result.Close()

	// Stored zeros, not scan artifacts: the comparison happens inside Kuzu,
	// before any Go type assertion can lose the value.
	m.ZeroTimestampObservations, err = countQuery(store, `
		MATCH (e:Entity {project_id: $project_id})-[:HAS_OBSERVATION]->(o:Observation)
		WHERE o.created_at IS NULL OR o.created_at = $zero
		RETURN count(*)
	`, map[string]any{"project_id": projectID, "zero": time.Time{}})
	if err != nil {
...
func countQuery(store *Store, query string, params map[string]any) (int, error) {
	...
	v, _ := row.GetValue(0)
	return intFromCount(v), nil
}

// intFromCount converts a Kuzu count(*) cell to int. Kuzu returns int64;
// uint64 is accepted defensively.
func intFromCount(v any) int {
	switch n := v.(type) {
	case int64:
		return int(n)
	case uint64:
