...
// The two must coexist: indexing after a hand-write leaves the journal holding
// exactly the hand-write, which is what lets `kg index` restore it.
func TestMCPJournalHoldsOnlyHandWritesAfterIndexing(t *testing.T) {
	handlers, dbPath := mcpTestEnv(t)
